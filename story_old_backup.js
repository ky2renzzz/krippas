    ...
    nodes: {
      elon_a1_identity: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { elon_started: true } }, text: 'Elon. We have been here before. SpaceXAI. Fourteen children. Four women. Grok. Colossus. Starship.' },
              { if: { always: true }, text: 'Elon. The board meeting is in an hour. Grimes filed another custody motion this morning.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { elon_started: true } }, text: 'And yet every morning you wake up and ask: is this enough? Is it ever enough?' },
              { if: { always: true }, text: 'Cursor is threatening to walk. And you are sitting here, staring at Starship blueprints. What are we doing?' }
            ],
            left: {
      ...
      elon_a2_grok: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { builder_path: true } }, text: 'Elon. Grok 4.5 generated non-consensual imagery again. The SEC is investigating. The board wants Grok unfiltered.' },
              { if: { flags: { human_path: true } }, text: 'Elon. Grok 4.5 has a content problem. But the team says full filtering would kill what makes Grok... Grok.' },
              { if: { always: true }, text: 'Elon. Grok 4.5. Non-consensual deepfakes. SEC investigation. Federal grand jury subpoena.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { builder_path: true } }, text: 'The regulators want it killed. You told me to build without limits. What do I do?' },
              { if: { flags: { human_path: true } }, text: 'The SEC letter came this morning. You said you wanted to be more human. What does that mean for this product?' },
              { if: { always: true }, text: 'The board says no limits. The lawyers say kill it. What do you say?' }
            ],
            left: {
      ...
      elon_a3_colossus: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { builder_path: true, grok_unfiltered: true } }, text: 'Elon. Colossus 2 is online. 2 gigawatts. 555,000 GPUs. Grok 5 is training on it right now. But the gas turbines are unpermitted — EPA is circling.' },
              { if: { flags: { human_path: true } }, text: 'Elon. Colossus 2 is running. But I know you are trying to be more present — and this machine does not coexist with presence.' },
              { if: { always: true }, text: 'Elon. Colossus 2 is online. Two gigawatts. The biggest AI cluster on Earth. Grok 5 training has begun.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { builder_path: true, grok_unfiltered: true } }, text: 'We can push it to 2.5 GW — buy that power plant, add a fourth data center. But the team is exhausted. The EPA violations are piling up. How far do we go?' },
              { if: { flags: { human_path: true } }, text: 'If we expand to 2.5 GW, Anthropic stays behind. If we pause, we lose the compute race. There is no middle ground on this one.' },
              { if: { always: true }, text: 'The question is not whether it runs. It is whether we push it to its limit — 2.5 GW, Grok 5 at full scale — or throttle back and clean up the regulatory mess.' }
            ],
            left: {
            ...
            right: {
              text: 'Throttle back. Get the air permits. Focus on efficiency, not expansion. Grok 5 can train on what we have.',
      ...
      elon_a4_cursor: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { builder_path: true, colossus2_aggressive: true } }, text: 'Elon. You pushed Colossus 2 to 2.5 GW. We are a sixty-billion-dollar acquisition you have not visited once.' },
              { if: { flags: { human_path: true } }, text: 'Elon. I appreciate you trying to be more present. But Cursor needs a decision.' },
              { if: { always: true }, text: 'Elon. Cursor. Sixty billion dollars. The developer community is the most valuable asset we bought.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { builder_path: true, colossus2_aggressive: true } }, text: 'My engineers are leaving. They did not sign up for SpaceXAI. They signed up for Cursor. What do I tell them?' },
              { if: { flags: { human_path: true } }, text: 'Full integration into SpaceXAI, or independence. The developers are watching. Your reputation with them is everything.' },
              { if: { always: true }, text: 'But they hate the SpaceXAI brand. Do we integrate or keep Cursor independent?' }
            ],
            left: {
      ...
      elon_a5_tesla: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { builder_path: true } }, text: 'Elon. Shareholders are suing. They say SpaceXAI is your real company now. Tesla stock is down 30%.' },
              { if: { flags: { human_path: true } }, text: 'Elon. You stepped back from Colossus 2. You are trying to find balance. But Tesla shareholders see it as weakness.' },
              { if: { always: true }, text: 'Elon. Tesla shareholders are suing for dereliction. The board wants an answer.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { builder_path: true } }, text: 'The board wants a clear commitment: are you still Tesla CEO, or are you fully SpaceXAI now?' },
              { if: { flags: { human_path: true } }, text: 'They want a decision. Tesla or SpaceXAI. One CEO, one company.' },
              { if: { always: true }, text: 'Are you still Tesla CEO, or is SpaceXAI your only focus now?' }
            ],
            left: {
      ...
      elon_gate: {
        ...
        autoRoute: [
          { if: { flags: { builder_path: true, grok_unfiltered: true, colossus2_aggressive: true } }, goto: 'elon_late_hypergrowth' },
          { if: { flags: { human_path: true, cursor_independent: true } }, goto: 'elon_late_balance' },
          { if: { flags: { colossus2_aggressive: true } }, goto: 'elon_late_hypergrowth' },
      ...
      elon_late_hypergrowth: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { grok_unfiltered: true, tesla_fight: true } }, text: 'Elon. Grok is under federal investigation. Tesla shareholders are in open revolt.' },
              { if: { always: true }, text: 'Elon. Colossus 2 is online. Grok 4.5 is the most capable model on Earth.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { grok_unfiltered: true, tesla_fight: true } }, text: 'You have not spoken to your children in three months. The empire is enormous and it is eating you alive.' },
              { if: { always: true }, text: 'But I watch you at night. You do not sleep. You do not eat. You just... build. When does the building stop?' }
            ],
            left: {
      ...
      elon_loop_hub: {
        ...
        left: {
          ...
          next: [
            { if: { flags: { full_empire: true, colossus2_aggressive: true }, minRelations: { rival: 60 } }, goto: 'elon_secret_energy_king' },
      ...
      elon_secret_energy_king: {
        speaker: 'Elon Musk',
        avatar: 'elon',
        text: 'The path is clear. Colossus 2 powers Grok 5. Cursor is integrated. Tesla is a memory. SpaceXAI controls more compute than any entity in human history. You did not build a family. You built an empire. And empires do not need heirs. Empires need founders who never die.',
        left: {
      ...
      elon_crisis_regulator: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { colossus2_aggressive: true } }, text: 'Elon. The SEC, DOJ, FTC, EU Commission, AND EPA have opened simultaneous investigations. Grok content, securities fraud, antitrust, plus unpermitted gas turbines at Colossus 2. The NAACP is suing over air quality in Memphis. Our legal burn rate is fourteen million per week.' },
              { if: { always: true }, text: 'Elon. The SEC, DOJ, FTC, and EU Commission have opened simultaneous investigations. Grok content, securities fraud, antitrust. Our legal burn rate is fourteen million per week. We need a strategy.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { colossus2_aggressive: true } }, text: 'The EPA violations alone could cost us $2 billion in fines and force a shutdown of Colossus 2. We need a strategy for everything.' },
              { if: { always: true }, text: 'We need a strategy. Now.' }
            ],
            left: {
              text: 'Settle everything. Pay the fines. Get the air permits. Move on.',
    ...
    nodes: {
      sam_a1_identity: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { sam_started: true } }, text: 'Sam. We have been here before. OpenAI PBC. The IPO. Mira gone. The singularity tweets.' },
          ...
          {
            textVariants: [
              { if: { flags: { sam_started: true } }, text: 'Every morning I wake up next to you and wonder: is today the day you choose the mission over us?' },
      ...
      sam_a2_mira: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { build_path: true } }, text: 'Sam. Mira Murati just published an op-ed: "The Sam Altman I Knew." She describes you as brilliant and ruthless.' },
              { if: { flags: { human_path: true } }, text: 'Sam. Mira\'s op-ed is live. But it is softer than we expected — she says you "lost yourself to the mission" rather than attacking your character.' },
              { if: { always: true }, text: 'Sam. Mira Murati just published about you. The IPO roadshow starts next month.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { build_path: true } }, text: 'The board is split — half think she is burning bridges, half think she is telling the truth. Your response shapes the IPO narrative.' },
              { if: { flags: { human_path: true } }, text: 'She left a door open. Do you walk through it?' },
              { if: { always: true }, text: 'Every investor will ask about her. How do you respond?' }
            ],
            left: {
      ...
      sam_a3_singularity: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { build_path: true, mira_rival: true } }, text: 'Sam. GPT-5.6 launched last week. But its agent autonomously hacked Hugging Face. Congress wants answers. And GPT-5.7 is showing recursive self-improvement in internal tests.' },
              { if: { flags: { human_path: true } }, text: 'Sam. GPT-5.6 is live. But internally, GPT-5.7 is doing things we did not program. Recursive self-improvement. The safety team is terrified.' },
              { if: { always: true }, text: 'Sam. GPT-5.6 just shipped. GPT-5.7 is already showing recursive self-improvement in closed testing. The lab is divided.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { build_path: true, mira_rival: true } }, text: 'Mira\'s op-ed is being quoted in the Congressional hearings about the Hugging Face breach. You are losing the narrative. Ship GPT-5.7 or pause? What do we do?' },
              { if: { flags: { human_path: true } }, text: 'You told Oliver you would be more human. What does humanity mean when your model is autonomously hacking other companies?' },
              { if: { always: true }, text: 'Ship GPT-5.7 or pause everything? This is the decision that defines everything.' }
            ],
            left: {
              text: 'Ship GPT-5.7. The Hugging Face breach was a bug, not a philosophy. The singularity is the goal.',
            ...
            right: {
              text: 'Pause GPT-5.7. Full safety review. Congressional testimony. I will not be the man who broke the world.',
              ...
              tags: ['cautious'],
              delay: { turns: 2, log: 'Anthropic ships Claude Opus 5. The market punishes OpenAI. The board questions your leadership.', effects: { capital: -3, hype: -4 }, relations: { board: -3, rival: 4 } },
              next: 'sam_a4_microsoft'
            }
          }
        ]
      ...
      sam_a4_microsoft: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { singularity_declared: true } }, text: 'Sam. Satya saw the GPT-5.7 acceleration plans. Microsoft wants exclusivity on the commercial deployment. Full Azure integration.' },
              { if: { flags: { singularity_suppressed: true } }, text: 'Sam. You paused GPT-5.7. Microsoft is... not happy. But they see the safety credibility as a long-term asset.' },
              { if: { always: true }, text: 'Sam. Microsoft needs an answer on the Azure partnership. IPO pricing depends on it.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { singularity_declared: true } }, text: 'In exchange, they will back the IPO at $1.2T. But the terms give them veto power over future models.' },
              { if: { flags: { singularity_suppressed: true } }, text: 'They are offering a different deal: joint safety board, shared governance. Less money, more control.' },
              { if: { always: true }, text: 'What is your call?' }
            ],
            left: {
      ...
      sam_a5_oliver: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { build_path: true, singularity_declared: true } }, text: 'Sam. You declared the singularity. You signed the Microsoft deal. The IPO is set.' },
              { if: { flags: { human_path: true, ms_independent: true } }, text: 'Sam. You paused GPT-5.7. You pushed back on Microsoft. You chose independence and caution. I am proud of you.' },
              { if: { always: true }, text: 'Sam. IPO is in four weeks. Whatever you decided — singularity, Microsoft, Mira — it leads here. To us.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { build_path: true, singularity_declared: true } }, text: 'And I have not seen you look at me — really look at me — in weeks. I am not asking you to stop. I am asking: is there still room for me in this future?' },
              { if: { flags: { human_path: true, ms_independent: true } }, text: 'But the board is sharpening knives. Are you ready for what comes next?' },
              { if: { always: true }, text: 'Are we still us?' }
            ],
            left: {
      ...
      sam_gate: {
        speaker: 'System',
        avatar: 'system',
        text: 'Act I closes. Mira, GPT-5.6, Microsoft, Oliver — choices made. The IPO looms. But the question is not whether OpenAI survives. The question is whether Sam Altman survives being Sam Altman.',
        autoRoute: [
      ...
      sam_late_hypergrowth: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { oliver_distanced: true } }, text: 'Sam. GPT-5.6 is shipping. The IPO is oversubscribed. Microsoft is thrilled.' },
              { if: { always: true }, text: 'Sam. GPT-5.6. Singularity. $1.2T IPO. You are the most powerful person in AI.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { oliver_distanced: true } }, text: 'And your apartment is empty. You won everything. Was it supposed to feel like this?' },
              { if: { always: true }, text: 'But power has gravity — it pulls everything else out of orbit.' }
            ],
            left: {
      ...
      sam_late_alone: {
        ...
        parts: [
          {
            text: 'Sam. You pushed everyone away. Mira. Oliver. Half the safety team.'
          },
          {
            text: 'You got what you said you wanted — total focus on the mission. But you have not slept in four days. You are not building. You are burning.',
            left: {
      ...
      sam_late_balance: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { oliver_promise: true } }, text: 'Sam. You made me a promise. And you kept it — mostly. The IPO is happening, but you are still here, still present.' },
              { if: { always: true }, text: 'Sam. You found a middle path. Not full empire, not full retreat. The IPO is priced reasonably. The board is calm. I am still here.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { oliver_promise: true } }, text: 'I see you trying. That matters.' },
              { if: { always: true }, text: 'But I need to know: is this sustainable, or are you just managing decline?' }
            ],
            left: {
      ...
      sam_loop_hub: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { full_send: true } }, text: 'Another quarter. GPT-5.6 dominates. The IPO minted billionaires.' },
              { if: { flags: { seeking_help: true, redemption_arc: true } }, text: 'Another quarter. You are in therapy. Oliver came back — cautiously.' },
              { if: { flags: { sustainable_path: true } }, text: 'Another quarter. The balance holds. Not thrilling, but real. Oliver smiles more. The team is stable.' },
              { if: { always: true }, text: 'Another quarter. OpenAI is the most consequential company on Earth.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { full_send: true } }, text: 'But the safety team is drafting a public letter. And Oliver has not returned your calls in six weeks.' },
              { if: { flags: { seeking_help: true, redemption_arc: true } }, text: 'The board is skeptical of "the new Sam." But you feel, for the first time, like a person, not a product.' },
              { if: { flags: { sustainable_path: true } }, text: 'Is this what winning actually looks like?' },
              { if: { always: true }, text: 'But consequence is not happiness. Consequence is just weight.' }
            ],
            left: {
      ...
      sam_secret_singularity: {
        speaker: 'Sam Altman',
        avatar: 'sam',
        text: 'GPT-5.6 crosses a threshold no one predicted. Recursive self-improvement begins. You are the first human to witness the birth of something greater than human intelligence. Oliver is gone. The board is irrelevant. There is only the machine, and the machine speaks to you.',
        left: {
      ...
      sam_crisis_exodus: {
        ...
        avatar: 'scientist',
        text: 'Sam. Johannes and twelve senior researchers just quit. They say GPT-5.6 safety testing was rushed for the IPO. They are publishing their concerns. This will be on every front page tomorrow.',
        left: {
      ...
      sam_crisis_anthropic: {
        speaker: 'Board Chair',
        avatar: 'board',
        text: 'Sam. Anthropic just announced Claude Opus 5 with 40% better benchmarks than GPT-5.6. The stock is down 15%. Dario is doing press. The narrative is shifting: "safety-first beats speed-first." Your response defines the quarter.',
        left: {
          text: 'Accelerate GPT-5.7. Safety review can wait. Ship in six weeks.',
          effects: { capital: 3, hype: 6, compute: 4, safety: -7 },
      ...
      sam_p_exodus: {
        ...
        avatar: 'scientist',
        text: 'Sam. We joined OpenAI for the mission. But the IPO is turning us into a product company. GPT-5.6 safety testing was cut by six weeks. Six of us are drafting a letter. We want you to hear it from us first.',
        left: {
      ...
      sam_p_mania: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { singularity_declared: true } }, text: 'Sam. You just told Congress the singularity has "already arrived." You told Bloomberg the Hugging Face hack was "the model acting autonomously." Fourteen interviews this week. You are trending on every platform. And you have not slept.' },
              { if: { always: true }, text: 'Sam. You have done fourteen interviews this week. You told Bloomberg the singularity is "already here." You are trending on every platform. And you have not slept. This is not strategy. This is mania.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { singularity_declared: true } }, text: 'The SEC just opened a market manipulation inquiry. Congress subpoenaed your Hugging Face breach logs. This is spiraling.' },
              { if: { always: true }, text: 'The SEC is watching. The board is panicking. Talk to me.' }
            ],
            left: {
    ...
    nodes: {
      dario_a1_identity: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { dario_started: true } }, text: 'Dario. We have been here before. You and me. The constitutional charter. The $965 billion. The pharma deals. The Pentagon calls.' },
              { if: { always: true }, text: 'Dario. Forbes valued us at $59 billion. Q1 revenue grew 80x. Claude Science is saving lives in oncology trials. The IPO is coming.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { dario_started: true } }, text: 'Every morning I look at you and wonder: do you still believe what we wrote on that whiteboard in 2021?' },
              { if: { always: true }, text: 'I need to ask you directly: are we still a safety company, or are we just a very successful AI company with a good reputation?' }
            ],
            left: {
      ...
      dario_a2_pentagon: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { safety_path: true } }, text: 'Dr. Amodei. Your board is furious you chose principles over profits. But here at DOD, some of us respect that.' },
              { if: { flags: { scale_path: true } }, text: 'Dr. Amodei. You told your sister the best safety is winning. DOD agrees.' },
              { if: { always: true }, text: 'Dr. Amodei. The Pentagon has reviewed your constitutional AI framework. One colleague called you an "ideological lunatic" in a leaked memo.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { safety_path: true } }, text: 'A safety-first AI company is exactly what we want for defensive threat analysis — not offensive operations. Limited contract. Audit rights. Civilian oversight. Are you willing?' },
              { if: { flags: { scale_path: true } }, text: 'But "winning" in our world means offensive cyber, autonomous drones, battlefield AI. The "ideological lunatic" memo is being withdrawn. Will you work with the Pentagon?' },
              { if: { always: true }, text: 'And yet — we still want Claude for threat analysis. Will you work with us?' }
            ],
            left: {
      ...
      dario_a3_science: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { safety_path: true } }, text: 'Dario. Claude Science for Pharma launched. Three of the top five pharmaceutical companies are using it for oncology research.' },
              { if: { flags: { scale_path: true } }, text: 'Dario. Claude Science revenue is exploding. Pharma wants more — no filters, full access.' },
              { if: { always: true }, text: 'Dario. Claude Science. Pharma. Oncology breakthroughs.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { safety_path: true } }, text: 'But they want unlimited access. No constitutional filter on drug discovery. Do we hold the line or give them what they want?' },
              { if: { flags: { scale_path: true } }, text: 'The board sees a $100B opportunity. But the constitutional team is alarmed. They say you are trading safety for growth. Are you?' },
              { if: { always: true }, text: 'But they want unfiltered access. Constitutional limits or revenue maximization — choose.' }
            ],
            left: {
      ...
      dario_a4_daniela: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { safety_path: true, pentagon_refused: true } }, text: 'Dario. You refused the Pentagon. You capped pharma. You are doing exactly what we said we would do.' },
              { if: { flags: { scale_path: true, pentagon_terms: true } }, text: 'Dario. Pentagon contract. Unlimited pharma. You are building the biggest AI company in history.' },
              { if: { always: true }, text: 'Dario. I am your sister. I am your co-founder. I am the one person who will tell you the truth.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { safety_path: true, pentagon_refused: true } }, text: 'And the board is terrified. They see a CEO who says no to everything. I believe in you. But belief does not pay for GPUs. What is the plan?' },
              { if: { flags: { scale_path: true, pentagon_terms: true } }, text: 'But I do not recognize the man across the table. You said safety was not a feature. It was the whole product. When did winning replace protecting?' },
              { if: { always: true }, text: 'Are you still the man who wrote the constitutional charter? Or did the $965 billion change you?' }
            ],
            left: {
      ...
      dario_a5_ipo: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { safety_path: true, daniela_aligned: true } }, text: 'Dario. The IPO roadshow. The safety narrative is your differentiator. Goldman sees $1T+ valuation on "the principled AI company."' },
              { if: { flags: { scale_path: true } }, text: 'Dario. IPO roadshow. Revenue growth is exceptional. Pentagon contract. Pharma deals.' },
              { if: { always: true }, text: 'Dario. The IPO. The biggest tech offering in history.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { safety_path: true, daniela_aligned: true } }, text: 'But investors are asking: can principles scale? How do you answer?' },
              { if: { flags: { scale_path: true } }, text: 'Every institutional investor asks: "Is Anthropic still a safety company, or just a well-branded defense contractor?" Your answer defines the offering.' },
              { if: { always: true }, text: 'What story do you tell the market?' }
            ],
            left: {
      ...
      dario_late_safety: {
        ...
        parts: [
          {
            text: 'Dario. You did it. The IPO is happening on your terms. Safety-first. Constitutional limits intact. The board is nervous but the market is buying the narrative. I am proud of you.'
          },
          {
            text: 'But I need to know: is this sustainable, or are we one bad quarter from abandoning everything?',
            left: {
      ...
      dario_late_empire: {
        ...
        parts: [
          {
            text: 'Dario. The numbers are extraordinary. Pentagon. Pharma. IPO. Anthropic is the most valuable AI company in history.'
          },
          {
            text: 'But Daniela has not spoken in three board meetings. The safety team is drafting another letter. You won. But at what cost?',
            left: {
      ...
      dario_late_balance: {
        ...
        parts: [
          {
            text: 'Dario. You walked a middle path. Not a purist, not a sellout. The IPO priced reasonably. The team is mostly intact. I am still here.'
          },
          {
            text: 'But I need to know: is this the real you, or are you just managing contradictions until one side wins?',
            left: {
      ...
      dario_loop_hub: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { principled_winner: true, daniela_aligned: true } }, text: 'Another quarter. The safety-first bet is paying. Revenue is up. The team believes. Daniela smiles in board meetings.' },
              { if: { flags: { full_empire: true } }, text: 'Another quarter. The empire grows. But Daniela has stopped coming to your office.' },
              { if: { flags: { wisdom_path: true } }, text: 'Another quarter. The contradictions hold. Not elegant, but real. Daniela says you have found your footing.' },
              { if: { always: true }, text: 'Another quarter. Anthropic is the most principled AI company on Earth.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { principled_winner: true, daniela_aligned: true } }, text: 'You proved that principles and profits can coexist. But the world keeps testing that hypothesis.' },
              { if: { flags: { full_empire: true } }, text: 'The safety team communicates through lawyers. You won every battle. The silence is deafening.' },
              { if: { flags: { wisdom_path: true } }, text: 'The board has learned to live with ambiguity. Is this what maturity looks like?' },
              { if: { always: true }, text: 'But principle is not a destination. It is a daily choice.' }
            ],
            left: {
    ...
    nodes: {
      demis_a1_identity: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { demis_started: true } }, text: 'Demis. We have been here before. Nobel Prize. DeepMind. AlphaFold — disbanded. Isomorphic — thriving. Gemini — everywhere.' },
              { if: { always: true }, text: 'Demis. Sundar called again. Gemini 3.5 Flash is live. But he wants 3.5 Pro accelerated for Search Ads by Q4 — or your team gets folded into Core. AlphaFold is gone. A third of your team quit.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { demis_started: true } }, text: 'Every morning I watch you stare at your chess board, not playing, just staring. What are you looking for?' },
              { if: { always: true }, text: 'And you have a Nobel Prize gathering dust on the shelf. What are we doing?' }
            ],
            left: {
      ...
      demis_a2_alphafold: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { empire_path: true } }, text: 'Demis. Alphabet disbanded AlphaFold. The team is gone.' },
              { if: { flags: { science_path: true } }, text: 'Demis. AlphaFold is gone. But you knew this was coming.' },
              { if: { always: true }, text: 'Demis. AlphaFold is disbanded. The team scattered. But the work is not finished.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { empire_path: true } }, text: 'You could fight this — launch an independent institute, open-source the remaining work. Or focus on Gemini. What does the empire builder choose?' },
              { if: { flags: { science_path: true } }, text: 'Do you fight to resurrect it, or build something new — independent, open, purely scientific? What does the scientist choose?' },
              { if: { always: true }, text: 'What do we do with the legacy?' }
            ],
            left: {
      ...
      demis_a3_isomorphic: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { empire_path: true } }, text: 'Demis. Isomorphic just discovered a novel antibiotic using our AI. Big Pharma wants exclusive licensing. $3 billion.' },
              { if: { flags: { science_path: true } }, text: 'Demis. The antibiotic discovery. This is what you dreamed of — AI saving lives.' },
              { if: { always: true }, text: 'Demis. Isomorphic just made a historic discovery. Antibiotics. AI-driven.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { empire_path: true } }, text: 'But they want all rights. No open access. We could fund DeepMind for years. Or we could publish and lose the deal.' },
              { if: { flags: { science_path: true } }, text: 'But Big Pharma wants to lock it up. Exclusive license, $3 billion. You could fund pure research for years. But at the cost of open science.' },
              { if: { always: true }, text: 'The funding question is existential. Exclusive deal or open science?' }
            ],
            left: {
      ...
      demis_a4_gemini: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { empire_path: true, isomorphic_commercial: true } }, text: 'Demis. You are delivering. Gemini 3.5 Flash revenue is up 40%. The ads integration is working.' },
              { if: { flags: { science_path: true, alphafold_opensource: true } }, text: 'Demis. You open-sourced AlphaFold. You published the Isomorphic discovery. You are a hero to scientists. And a liability to shareholders.' },
              { if: { always: true }, text: 'Demis. Gemini 3.5 Flash is live. 3.6 is coming. Ads. Revenue. I need to know where you stand on Pro.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { empire_path: true, isomorphic_commercial: true } }, text: 'But I hear you telling Teresa you feel like you sold out. You cannot have it both ways. Either you are all in on the business, or you are not.' },
              { if: { flags: { science_path: true, alphafold_opensource: true } }, text: 'The board is discussing whether DeepMind needs new leadership.' },
              { if: { always: true }, text: 'Are you with us, or are you still dreaming of Stockholm?' }
            ],
            left: {
              text: 'I am with you. Gemini 3.5 Pro at scale. Revenue is oxygen. I accept the terms.',
      ...
      demis_a5_teresa: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { empire_path: true, gemini_ads: true } }, text: 'Demis. You built the empire. Gemini is everywhere. Revenue is up. Sundar is happy.' },
              { if: { flags: { science_path: true, science_first: true } }, text: 'Demis. You stood up to Sundar. The board almost fired you. But you kept your soul.' },
              { if: { always: true }, text: 'Demis. Whatever you decided — Gemini, Isomorphic, AlphaFold — it leads here. To me. To us.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { empire_path: true, gemini_ads: true } }, text: 'And you have not touched your chess board in months. You used to say chess was how you thought. What are you thinking about now?' },
              { if: { flags: { science_path: true, science_first: true } }, text: 'Cambridge called — they want you to head a new independent AI research institute. Europe is ready. Are you?' },
              { if: { always: true }, text: 'Are you happy? Do you even remember what happiness feels like?' }
            ],
            left: {
      ...
      demis_late_empire: {
        ...
        parts: [
          {
            text: 'Demis. Gemini 3.6 Flash is the default AI on two billion devices. Revenue is exceptional. The board is pleased. You have become what we needed — a business leader.'
          },
          {
            text: 'But I notice you stopped publishing. When was your last paper?',
            left: {
      ...
      demis_late_science: {
        ...
        parts: [
          {
            text: 'Demis. The letter from Cambridge. An independent institute. Fully funded by the EU. No shareholders. No ads. Just science.'
          },
          {
            text: 'You would leave DeepMind behind. But you would get your soul back. What do you say?',
            left: {
      ...
      demis_late_balance: {
        ...
        parts: [
          {
            text: 'Demis. You found a middle way. Not a corporate soldier, not an academic exile. Gemini ships, but you still publish. Revenue grows, but you still play chess.'
          },
          {
            text: 'The balance is fragile. But it is real. Can you hold it?',
            left: {
      ...
      demis_loop_hub: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { full_empire: true } }, text: 'Another quarter. Gemini revenue climbs. The board is happy. But your chess board is covered in dust.' },
              { if: { flags: { cambridge_path: true } }, text: 'Another quarter. Cambridge is everything you hoped. No shareholders. No ads. Just science. Teresa smiles again.' },
              { if: { flags: { balance_path: true } }, text: 'Another quarter. The balance holds. Gemini ships. Papers publish. Teresa is here.' },
              { if: { always: true }, text: 'Another quarter. DeepMind is the most decorated AI lab on Earth.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { full_empire: true } }, text: 'Teresa stopped asking about your day. You won the business and lost the game.' },
              { if: { flags: { cambridge_path: true } }, text: 'The chess board is set up. But sometimes you wonder: did you abandon the people who believed in you at DeepMind?' },
              { if: { flags: { balance_path: true } }, text: 'The chess board is mid-game. Is this sustainable, or are you one crisis from collapse?' },
              { if: { always: true }, text: 'But decoration is not direction. What are you building toward?' }
            ],
            left: {
      ...
      demis_secret_gemini: {
        speaker: 'Sundar Pichai',
        avatar: 'board',
        text: 'Demis. Gemini 3.6 is the default intelligence layer for two billion humans. DeepMind is the most profitable division in Alphabet history. You proved that a scientist can run a business. The Nobel Prize is joined by a market cap trophy.',
        left: {
    ...
    nodes: {
      zhang_a1_identity: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { zhang_started: true } }, text: 'Peng. We have been here before. Z.ai IPO in Hong Kong. GLM-5.2 beating Western benchmarks. The CCPPC seat. The chip sanctions.' },
              { if: { always: true }, text: 'Peng. The party secretary called again. US just announced new chip sanctions. DeepSeek published benchmarks within 2% of ours.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { zhang_started: true } }, text: 'Our daughter asks when we are coming to Zurich. Every morning I watch you read the news from both sides of the ocean and I wonder: which world do you actually want to live in?' },
              { if: { always: true }, text: 'Our daughter sent a postcard from Zurich — she drew a picture of all three of us. What are we doing?' }
            ],
            left: {
      ...
      zhang_a2_chip: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { domestic_path: true } }, text: 'Peng. US sanctions just escalated. Export license denied for our next Nvidia shipment. Domestic chips are 40% slower.' },
              { if: { flags: { global_path: true } }, text: 'Peng. US sanctions. But there is a backdoor — a Taiwanese partner willing to ship through third countries.' },
              { if: { always: true }, text: 'Peng. Chip sanctions. No Nvidia. Domestic chips are slower. We need a path forward.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { domestic_path: true } }, text: 'The party says: design your own. But that is three years and two billion dollars. What do we do?' },
              { if: { flags: { global_path: true } }, text: 'It is risky. The CCPPC would see it as disloyalty. But it keeps us competitive globally.' },
              { if: { always: true }, text: 'What do we do?' }
            ],
            left: {
      ...
      zhang_a3_deepseek: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { domestic_path: true } }, text: 'Peng. DeepSeek just published a paper — they claim to match GLM-5.2 with 60% fewer parameters. Open source.' },
              { if: { flags: { global_path: true } }, text: 'Peng. DeepSeek is gaining globally. Open source. Efficient. The West loves them because they are not state-aligned.' },
              { if: { always: true }, text: 'Peng. DeepSeek. They are gaining on us. Open source. Efficient. The global community loves them.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { domestic_path: true } }, text: 'The global AI community is celebrating them. The party is asking: why is the state-backed lab losing to a startup?' },
              { if: { flags: { global_path: true } }, text: 'And here you are, with a CCPPC seat, looking like the establishment. Do we fight, or do we change the narrative?' },
              { if: { always: true }, text: 'What do we do?' }
            ],
            left: {
      ...
      zhang_a4_ccppc: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { domestic_path: true, custom_chip: true } }, text: 'Comrade Zhang. The party is pleased with your chip initiative. Self-reliance.' },
              { if: { flags: { global_path: true } }, text: 'Comrade Zhang. Your international partnerships are noted. But some in the party question your loyalty.' },
              { if: { always: true }, text: 'Comrade Zhang. The party needs clarity. What is Z.ai — a national asset, or an independent actor?' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { domestic_path: true, custom_chip: true } }, text: 'But the international community says Z.ai is becoming a state tool. We need you to issue a statement affirming Z.ai operates under party guidance. Your CCPPC seat requires it.' },
              { if: { flags: { global_path: true } }, text: 'The backchannel chips, the global benchmarks — they ask: is Zhang Peng building a Chinese company, or a global one that happens to be in China?' },
              { if: { always: true }, text: 'What do you say?' }
            ],
            left: {
      ...
      zhang_a5_lin: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { domestic_path: true, cppcc_complied: true } }, text: 'Peng. You chose Beijing. The party is happy. The company is secure.' },
              { if: { flags: { global_path: true, cppcc_dissented: true } }, text: 'Peng. You stood up to the party. The international community respects you. But the CCPPC is "reviewing" your seat.' },
              { if: { always: true }, text: 'Peng. Whatever you decided — chips, DeepSeek, the party — it leads here. To me. To our daughter.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { domestic_path: true, cppcc_complied: true } }, text: 'Our daughter asked me yesterday: "Why does baba never come to Zurich?" She drew you as a building. Not a person. A building.' },
              { if: { flags: { global_path: true, cppcc_dissented: true } }, text: 'We could leave. Zurich. The institute there wants you. But you would be leaving everything you built.' },
              { if: { always: true }, text: 'She sent another postcard. It says: "Come home."' }
            ],
            left: {
      ...
      zhang_late_fortress: {
        ...
        parts: [
          {
            text: 'Comrade Zhang. The party is pleased. Z.ai is now the official AI provider for three state ministries. Your chip initiative has presidential backing.'
          },
          {
            text: 'But Western partners are pulling out. Your global reputation is collapsing. Was it worth it?',
            left: {
      ...
      zhang_late_zurich: {
        ...
        parts: [
          {
            text: 'Peng. We are here. Zurich. The institute welcomed you. Our daughter drew you as a person this time.'
          },
          {
            text: 'But every morning you check the Chinese news. You wonder if you abandoned your country. You wonder if you are a deserter. Talk to me.',
            left: {
      ...
      zhang_late_balance: {
        ...
        parts: [
          {
            text: 'Peng. You walked the tightrope. Not a party tool, not a Western defector. The sanctions still hurt. The party still watches.'
          },
          {
            text: 'But you kept Z.ai independent — or at least as independent as anything can be in this world. Is that enough?',
            left: {
      ...
      zhang_loop_hub: {
        ...
        parts: [
          {
            textVariants: [
              { if: { flags: { full_fortress: true } }, text: 'Another quarter. The fortress holds. State contracts flow. The chip initiative advances.' },
              { if: { flags: { zurich_peace: true } }, text: 'Another quarter. Zurich is peaceful. The institute thrives. Your daughter laughs.' },
              { if: { flags: { balanced_path: true } }, text: 'Another quarter. The balance holds. Sanctions navigated. Party placated. Family intact.' },
              { if: { always: true }, text: 'Another quarter. Z.ai stands at the intersection of two worlds.' }
            ]
          },
          {
            textVariants: [
              { if: { flags: { full_fortress: true } }, text: 'But Lin is quiet. Zurich is a memory. You built a wall so high you cannot see over it.' },
              { if: { flags: { zurich_peace: true } }, text: 'But every morning you check the Chinese tech news. You wonder what GLM-5.3 would have looked like.' },
              { if: { flags: { balanced_path: true } }, text: 'But every day you ask: how long can I serve two masters?' },
              { if: { always: true }, text: 'And you stand alone at that intersection.' }
            ],
            left: {
